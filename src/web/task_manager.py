...
class TaskManager:
    ...
    @staticmethod
    def get_active_single_task() -> Optional[dict]:
        """获取当前活跃的单体任务（排除批量任务中的子任务）"""
        for task_uuid, status in _task_status.items():
            if not status.get("finished", False) and not status.get("is_subtask", False):
    ...
    def register_websocket(self, task_uuid: str, websocket):
        ...
        with _ws_lock:
            ...
            if websocket not in _ws_connections[task_uuid]:
                ...
                logger.debug(f"WebSocket 监听已注册: {task_uuid}")
            else:
                logger.debug(f"WebSocket 连接已存在，跳过重复注册: {task_uuid}")
    ...
    def update_status(self, task_uuid: str, status: str, **kwargs):
        ...
        if self._loop and self._loop.is_running():
            try:
                # kwargs 中已经包含了除了 status 之外的其他元数据（如 email）
                asyncio.run_coroutine_threadsafe(
    ...
    def init_batch(self, batch_id: str, total: int, description: str = ""):
        """初始化批量任务"""
        if batch_id in _batch_status:
            # 如果已存在且之前的总数是 0，现在有了具体总数，则补充打印日志
            old_total = _batch_status[batch_id].get("total", 0)
            _batch_status[batch_id]["total"] = total
            if description:
                _batch_status[batch_id]["description"] = description
            if old_total == 0 and total > 0:
                logger.info(f"批量任务 {batch_id} 总数已确认: {total}")
            return
        ...
        # 如果初始总数就是 0 (通常是预注册阶段)，暂时不打印日志，等确认后再打印
        if total > 0:
    ...
    def register_batch_websocket(self, batch_id: str, websocket):
        ...
        with _ws_lock:
            ...
            if websocket not in _ws_connections[key]:
                ...
                logger.debug(f"批量任务 WebSocket 已注册: {batch_id}")
            else:
                logger.debug(f"批量任务 WebSocket 连接已存在: {batch_id}")
