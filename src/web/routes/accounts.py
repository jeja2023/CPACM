...
async def list_accounts(
    ...
    cpa_uploaded: Optional[bool] = Query(None, description="CPA 上传状态筛选"),
):
    """
    获取账号列表
    ...
    with get_db() as db:
        ...
        # CPA 上传状态筛选
        if cpa_uploaded is not None:
            query = query.filter(Account.cpa_uploaded == cpa_uploaded)
...
async def batch_refresh_tokens(request: BatchRefreshRequest, background_tasks: BackgroundTasks):
    ...
    async def _refresh_one(account_id: int):
        try:
            result = await loop.run_in_executor(None, do_refresh, account_id, proxy)
            return account_id, result.success, result.error_message
        except Exception as e:
    ...
    async def _refresh_with_limit(account_id: int):
        async with semaphore:
            return await _refresh_one(account_id)

    tasks = [_refresh_with_limit(aid) for aid in ids]
    ...
    for outcome in outcomes:
        if isinstance(outcome, Exception):
            ...
            account_id, success, error_msg = outcome
            if success:
                results["success_count"] += 1
            else:
                results["failed_count"] += 1
                results["errors"].append({"id": account_id, "error": error_msg})
...
async def batch_validate_tokens(request: BatchValidateRequest):
    ...
    async def _validate_one(account_id: int):
        try:
            is_valid, error = await loop.run_in_executor(None, do_validate, account_id, proxy)
            return account_id, is_valid, error
        except Exception as e:
            return account_id, False, str(e)

    # 并发上限，避免同时打出几百个请求把代理/服务器压垮
    concurrency = 20
    semaphore = asyncio.Semaphore(concurrency)

    async def _validate_with_limit(account_id: int):
        async with semaphore:
            return await _validate_one(account_id)

    tasks = [_validate_with_limit(aid) for aid in ids]
    outcomes = await asyncio.gather(*tasks, return_exceptions=True)

    for outcome in outcomes:
        if isinstance(outcome, Exception):
            results["invalid_count"] += 1
            results["details"].append({"valid": False, "error": str(outcome)})
        else:
            account_id, is_valid, error = outcome
