...
class OpenAIHTTPClient(HTTPClient):
    ...
    def check_ip_location(self):
        """
        检查出口 IP 是否属于 OpenAI 支持的国外区域
        """
        try:
            # 换用 Cloudflare 的全球通用检测点
            response = self.get("https://1.1.1.1/cdn-cgi/trace", timeout=5)
            trace_text = response.text

            # 解析位置信息 (例如 loc=US, loc=SG, loc=JP)
            import re
            loc_match = re.search(r"loc=([A-Z]+)", trace_text)
            loc = loc_match.group(1) if loc_match else "UNKNOWN"

            # OpenAI 严控区域黑名单 (CN: 大陆, HK: 香港, MO: 澳门, RU: 俄罗斯)
            # 注意：虽然 HK 在 CF 可能是直连，但 OpenAI 100% 封锁 HK
            if loc in ["CN", "HK", "MO", "RU", "KP", "IR"]:
