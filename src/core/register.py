...
class RegistrationEngine:
    ...
    def _log(self, message: str, level: str = "info"):
        ...
        lvl_tag = tags.get(level.lower(), level.upper())
        # 控制台实时显示的消息格式
        log_message = f"[{lvl_tag}] {message}"
    ...
    def _get_device_id(self) -> Optional[str]:
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                self._log(
                    f"获取 Device ID 失败: 未返回 oai-did (HTTP {response.status_code}, 第 {attempt}/{max_attempts} 次尝试)",
                    "warning" if attempt < max_attempts else "error"
                )
            except Exception as e:
                self._log(
                    f"获取 Device ID 异常: {e} (第 {attempt}/{max_attempts} 次尝试)",
                    "warning" if attempt < max_attempts else "error"
                )
            ...
            try:
                ...
                if is_existing:
                    ...
                    if record_existing_account:
                        self._log(f"检测到已存在账号; 自动切换至登录流程")
                        self._is_existing_account = True
                    else:
                        self._log("登录流程已初始化; 正在等待验证码...")
