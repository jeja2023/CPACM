...
let currentFilters = { status: '', email_service: '', search: '', cpa_uploaded: null };  // 当前筛选条件
...
async function loadAccounts() {
    ...
    try {
        ...
        currentFilters = {
            ...
            search,
            cpa_uploaded: filterUnuploaded ? false : null
        };
...
function buildBatchPayload(extraFields = {}) {
    if (selectAllPages) {
        return {
            ...
            cpa_uploaded_filter: currentFilters.cpa_uploaded,
            ...extraFields
        };
    }
...
async function exportAccounts(format) {
    ...
    if (count === 0) {
        ...
        payload = {
            ...
            cpa_uploaded_filter: elements.filterCpaToggle.checked ? false : null
        };
    }
